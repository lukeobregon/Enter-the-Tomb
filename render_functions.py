...
def render_bar(panel, x, y, total_width, name, value, maximum, bar_color, back_color): #creates a general function to pass any information onto console screen
...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height, bar_width, panel_height, panel_y, mouse, colors, game_state):
    ...
    #print the game messages, on line at a time
    y = 1
...
def draw_entity(con, entity, fov_map, game_map):
    #does the actual drawing using anything we pass through it and also keeps stairs on the map after discovery
    if libtcod.map_is_in_fov(fov_map, entity.x, entity.y) or (entity.stairs and game_map.tiles[entity.x][entity.y].explored):
        libtcod.console_set_default_foreground(con, entity.color)
        libtcod.console_put_char(con, entity.x, entity.y, entity.char, libtcod.BKGND_NONE)

def clear_entity(con, entity):
    #erase te character that represents this object
